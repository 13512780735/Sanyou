...
public class CircleProgressBar extends View {
    ...
    private boolean mDisplayBottom;
    ...
    public CircleProgressBar(final Context context, AttributeSet attrs, int defStyle) {
        ...
        mDisplayBottom=mTypedArray.getBoolean(R.styleable.CircleProgressBar_bottom_content,true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mDisplayBottom && mStyle == STROKE) {
            mPaint.setStrokeWidth(0);
            mPaint.setColor(mTextColor);
            mPaint.setTextSize(mTextSize);
            mPaint.setTypeface(Typeface.DEFAULT);
            int percent1 = (int) (((float) mProgress / (float) mMaxProgress) * 100);
            float textWidth1= mPaint.measureText("电量");

            if (percent1 != 0) {
                canvas.drawText("电量", center - textWidth1 / 2, center + mTextSize / 2+30, mPaint);
            }
        }
        /**
         * 画进度百分比
